...
def checkDailyFile(path):
	...
	if stat > 2:
		return None	
	elif len(operV) < configSFpark.TIME_RESOLUTION: 
		return None 
	else:	
		v1 = np.array(occV)
		v2 = np.array(operV)		
...
def plotCentroids():
	...
	NUM_CLUSTER = 30
	centroids = getpickleFile('k30centroids.pickle')
	...
	for col in range(NUM_CLUSTER):  
		...
		fig = plt.figure()
		plt.plot(x, centroid, 'o-')
		plt.xlabel('hourly shape in one day')
		plt.ylabel('occupancy rate')
		#plt.show()
		plt.ylim(0,1)
		plt.savefig('shape_'+str(col)+'.png')
