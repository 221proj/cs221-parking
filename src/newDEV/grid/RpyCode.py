...
def show(v1, v2):
	...
	# robjects 	
	fitObj = robjects.r(" occfit<-arima(occv, order=c(2,0,0), seasonal=list(order=c(0,1,0), period=24 ), include.mean=FALSE )")
	occPred = robjects.r(" occpred<-predict(occfit, n.ahead=1)")
	#print fitObj.names
